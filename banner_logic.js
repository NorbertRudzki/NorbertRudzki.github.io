...
from 'https://ocs-pl.oktawave.com/v1/AUTH_0766c9f9-3aa7-4a69-83ac-7625429eb50e/ListonicStatic/assets/Templates/logic/adUtils.js'

function showBanner() {
    // HANDLE CSS
    const hasButton = Settings.HasButton;
    // insert default css
    insertCss('https://ocs-pl.oktawave.com/v1/AUTH_0766c9f9-3aa7-4a69-83ac-7625429eb50e/ListonicStatic/assets/Templates/styles/banner_style.css');
    // modify css if ATL banner
    if (hasButton) {
        mraid.addEventListener('changeATLBackgroundColor', changeATLBackgroundColor);
	...
	const cpm = parseFloat(Settings.CPM);
    ...
    if (isExpand) {
        ...
        mraid.sasSendMessage("canExpand_" + adViewWidth + "x" + realBannerHeight + "_" + adViewWidth + "x" + expandBannerHeight);
    }
	mraid.sasSendMessage("CPM_" + cpm);
};
